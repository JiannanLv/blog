...
//		data.neirong= this.neirong.value;
//		data.upload= this.file.value;
//      console.log(data);
		let data="title="+this.title.value+"&neirong="+this.neirong.value+"&upload="+this.file.value;
		...
		return (
			<form className="publish" ref={el=>{this.form=el}} >
    			...
    			<div className="publishItem">
      				...
      				<div className="inputDiv">
        				<textarea className="textarea" placeholder="请输入文章内容" ref={el=>{this.neirong=el}} rows="10" name="neirong"  placeholder="最少20个字"></textarea>
      				</div>
    			</div>
    			<div className="publishItem">
    			...
    			<button type="button" className="btn publishBtn" onClick={()=>{this.handleSubmit()}} >发布文章</button>
    		</form>
    	);
	}
}
