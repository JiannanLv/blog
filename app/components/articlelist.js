...
class PageLi extends Component{
	swtichPage(){
  	...
  	render(){
  		...
  		console.log(currenNum);
  		let type=this.props.type,pathname="",query={},keyword=this.props.keyword;
  		if(type=="index"){
  			pathname="/";
  			query={ page: currenNum };
  		}
  		if(type=="search"){
  			pathname="search";
  			query={ page: currenNum,keyword:keyword };
  		}
    return (
    	<li onClick={()=>{this.swtichPage()}}>
    		<Link to={{ pathname: pathname, query: query }} activeClassName="active" >{currenNum}</Link>
    	</li>
    );
  }
}
const ArticleList=({articleList,articleAction,type,keyword})=>{
    ...
    if(articleList&&articleList.data&&articleList.data.length){
    	...
    	//页码最大条数
    	pageNum=Math.ceil(page.count/page.limitNum);
  	...
  	return (
  		<div>
  			<ul className="articleList">{articleTmp.length?articleTmp:"无搜索结果"}</ul>
